...
# ── AUTO-TRAIN on startup (runs once, cached) ──────────────────────────────────
def ensure_trained():
    """Load data, preprocess, train all models, and store in session_state.
    Runs automatically on every page — cached so it only trains once per session."""
    if "trained" not in st.session_state:
        with st.spinner("⚙️ Initialising models… this takes ~30 seconds on first load."):
            raw = load_raw()
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    best_reg_name = st.session_state["best_reg"]
...
# ══════════════════════════════════════════════════════════════════════════════
# PAGE 4 — PREDICT DEMAND (calendar-synced)
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with st.form("pred_form"):
        ...
        with col2:
            ...
            members  = int(adults) + int(children)
            st.markdown(
                f"<div style='background:#0a1628;border:1px solid #1a2e4a;border-radius:8px;"
                f"padding:.45rem .9rem;font-size:.82rem;color:#5ba3d9;margin-bottom:.4rem'>"
                f"👨\u200d👩\u200d👧 <b style=\'color:#c8dcf0\'>Family Members: {members}</b>"
            ...
            income = st.number_input(
                ...
                step=500,
                key="p_income",
                help="Type any amount or use the spinner. Range: ₹1,000 – ₹5,00,000."
            )
